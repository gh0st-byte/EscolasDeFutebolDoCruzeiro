...
function initFormularioContato() {
  ...
  form.addEventListener('submit', function(e) {
    ...
    if (!nome || !cidade  || !bairro || !estado || !unidade || !telefone) {
...
// Função para salvar dados e enviar por mailto
function salvarDadosLicenciado(dados) {
  ...
  const assunto = 'Nova Solicitação de Licenciamento - Escolas do Cruzeiro';
  const corpo = `Nova Solicitação de Licenciamento\n\nData: ${new Date().toLocaleString('pt-BR')}\n\nNome: ${dados.nome}\nE-mail: ${dados.email}\nWhatsApp: ${dados.telefone}\nCidade de interesse: ${dados.cidade}\nEstado: ${dados.estado}\n${dados.experiencia ? `Experiência: ${dados.experiencia}\n` : ''}${dados.investimento ? `Capacidade de investimento: ${dados.investimento}\n` : ''}${dados.mensagem ? `\nMensagem:\n${dados.mensagem}\n` : ''}\nEnviado através do site das Escolas do Cruzeiro`;
  
  // Abrir cliente de email
  const mailtoLink = `mailto:marco.paiva.repoles01@gmail.com?subject=${encodeURIComponent(assunto)}&body=${encodeURIComponent(corpo)}`;
...
function initFormularioLicenciado() {
  ...
  form.addEventListener('submit', function(e) {
    ...
    // Salvar dados e abrir email
    salvarDadosLicenciado(dados)
      .then(response => {
        if (response.ok) {
          alert('Dados salvos! Seu cliente de email será aberto para envio.');
          form.reset();
        }
      })
      .finally(() => {
